...
2. Make sure you have `NODE=22` (specified in `.node-version`) and `pnpm=10.6.1` (specified in `package.json`) installed on your local machine (or docker/VM) before running the dev server. To know how to run the dev server please refer to `README.md`.
